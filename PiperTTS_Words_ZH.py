...
with open(wordFile,"r",encoding='utf-8') as file:
    wordCount       = 0
    create_folder("wav/"+FileName+'/translations')
    create_folder("wav/"+FileName+'/words_cn')
    for i, line in enumerate(file):
        if i % 5 == 0 and not CHECK_FILE:
            ...
            for mean in means:
                file_path="wav/"+FileName+'/words_cn/'+mean+'.wav'
                if not check_file(file_path) and not CHECK_FILE:
                    generate_wav(mean, file_path)
        elif i % 5 == 3 and not CHECK_FILE:
            continue
        elif i % 5 == 4 and not CHECK_FILE:
            filename = line.strip()
            clean_filename = sanitize_filename(filename)
            file_path="wav/"+FileName+'/translations/'+clean_filename+'.wav'
