...
with open(wordFile,"r",encoding='utf-8') as file:
    wordCount       = 0
    create_folder("wav/"+FileName+'/words_uk')
    create_folder("wav/"+FileName+'/sentences_uk')
    for i, line in enumerate(file):
        if i % 5 == 0 and not CHECK_FILE:
            filename = line.strip()
            clean_filename = sanitize_filename(filename)
            file_path="wav/"+FileName+'/words_uk/'+clean_filename+'.wav'
            ...
            filename = line.strip()
            clean_filename = sanitize_filename(filename)
            file_path="wav/"+FileName+'/sentences_uk/'+clean_filename+'.wav'
            if not check_file(file_path):
                generate_wav(filename, file_path)
